...
with open(os.path.join(MCdir,'sr_MCmean.npz'),'wb') as data:
    np.savez_compressed(data, srmean=srmean)
with np.load(os.path.join(MCdir,'sr_MCmean.npz'),'rb') as data:
    srmean=data['srmean']
...
for i,f in enumerate(reversed(files)):
    ...
    reala=imagenorm(diffa[None,...]/255.)
